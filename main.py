...
async def on_message(message):
    ...
    if message.author.id in afk_users:
        if not message.content.startswith('ash.afk'):
    ...
    # ash.afk
    if message.content.startswith('ash.afk'):
        alasan = message.content[7:].strip() or 'Tidak ada alasan'
    ...
    # ash.play
    if message.content.startswith('ash.play '):
        query = message.content[9:]
        await message.delete()
        vc = message.guild.voice_client
        if not vc:
            msg = await message.channel.send('Bot belum di voice channel! Ketik `ash.join` dulu.')
            await asyncio.sleep(5)
            await msg.delete()
            return
